...
@Composable
fun LoginWithOtpScreen(context: Context, navController: NavController, viewModel: LoginWithOtpViewModel) {
    var mobilNumber by remember { mutableStateOf("") }
    ...
    Scaffold(
        content = {
            Surface(
                ...
                when(result) {
                    is Result.Success -> {
                        val args = OtpArgs((result as Result.Success<LoggedInWithOtpUserView>).data.phoneNumber, CompletionNavigation.Home)
                        val action = LoginWithOtpFragmentDirections.actionLoginWithOtpFragmentToVerifyOtpFragment(args)
                ...
                Column(
                    ...
                    DefaultOutlinedTextField(
                        40.dp,
                        text = mobilNumber,
                        onValueChange = { mobilNumber = it },
                        label = stringResource(id = R.string.prompt_your_mobile_number),
                        isError = loginUiState.mobileNumberError.hasValue,
                        singleLine = true,
                        errorText = loginUiState.mobileNumberError.errorString,
